...
import { QUOTE_STATUS_LABELS } from '@/lib/documentStatus';
...
interface QuoteFormProps extends FormDirtyProps {
  quote?: Quote;
  initialClientId?: string;
  customFieldValues?: Record<string, unknown>;
  onSubmit: (data: QuotePayload, customFields: Record<string, unknown>) => Promise<void>;
  onCancel: () => void;
  embedded?: boolean;
}
...
export function QuoteForm({
  ...
  const [clientId, setClientId] = useState(quote?.clientId ?? initialClientId ?? '');
  ...
  const taxDefault = quote?.taxRate ?? defaultTaxRate;
    ...
    <form onSubmit={handleSubmit(submit)} className="space-y-5">
      ...
      <div className={cn('flex justify-end gap-2 pt-2', embedded && 'border-t border-border/60')}>
        ...
        <Button type="submit" loading={isSubmitting} disabled={editMode}>
          {quote ? 'Guardar' : 'Crear presupuesto'}
        </Button>
      </div>
    </form>
  );
